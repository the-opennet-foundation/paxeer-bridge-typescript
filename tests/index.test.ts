...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['PAXEER_BRIDGE_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: PaxeerBridge) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new PaxeerBridge({ walletSignature: 'My Wallet Signature' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['PAXEER_BRIDGE_LOG'] = 'debug';
      const client = new PaxeerBridge({ logger: logger, walletSignature: 'My Wallet Signature' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['PAXEER_BRIDGE_LOG'] = 'not a log level';
      const client = new PaxeerBridge({ logger: logger, walletSignature: 'My Wallet Signature' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'PAXEER_BRIDGE_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['PAXEER_BRIDGE_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['PAXEER_BRIDGE_BASE_URL'] = 'https://example.com/from_env';
      const client = new PaxeerBridge({ walletSignature: 'My Wallet Signature' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['PAXEER_BRIDGE_BASE_URL'] = ''; // empty
      const client = new PaxeerBridge({ walletSignature: 'My Wallet Signature' });
      expect(client.baseURL).toEqual('https://sidiora.exchange/api/bridge/');
    });

    test('blank env variable', () => {
      process.env['PAXEER_BRIDGE_BASE_URL'] = '  '; // blank
      const client = new PaxeerBridge({ walletSignature: 'My Wallet Signature' });
      expect(client.baseURL).toEqual('https://sidiora.exchange/api/bridge/');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new PaxeerBridge({ walletSignature: 'My Wallet Signature', environment: 'production' }),
    ...
    test('in request options overridden by env variable', () => {
      process.env['PAXEER_BRIDGE_BASE_URL'] = 'http://localhost:5000/env';
      const client = new PaxeerBridge({ walletSignature: 'My Wallet Signature' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new PaxeerBridge({ maxRetries: 4, walletSignature: 'My Wallet Signature' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new PaxeerBridge({ walletSignature: 'My Wallet Signature' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['PAXEER_BRIDGE_WALLET_SIGNATURE'] = 'My Wallet Signature';
    const client = new PaxeerBridge();
    expect(client.walletSignature).toBe('My Wallet Signature');
  });

  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['PAXEER_BRIDGE_WALLET_SIGNATURE'] = 'another My Wallet Signature';
    const client = new PaxeerBridge({ walletSignature: 'My Wallet Signature' });
    expect(client.walletSignature).toBe('My Wallet Signature');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new PaxeerBridge({ walletSignature: 'My Wallet Signature', fetch: testFetch });
